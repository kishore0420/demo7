...
steps {
sh 'docker build -t $kishore:$BUILD_NUMBER .'
}
}
...
steps {
sh 'docker run -d --name $demo7$BUILD_NUMBER -p $BUILD_NUMBER:8080 --restart unless-stopped $kishore:$BUILD_NUMBER && docker ps'
}
}
